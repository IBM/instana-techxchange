...
Now, the configuration.yaml file is ready to be used. The next step is to install the Instana agent using the altered configuration. Please run below command:
...
:::warning
Please note, that you have to update your Agent and Download Key values. Please confirm, if the values are the same as in the Instana UI -> Deploy Agent -> search for Docker
:::
