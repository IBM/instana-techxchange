...
Login to the "docker" Virtual Machine by using the following command
...
To enable the reception of the OpenTelemetry data in the host agent, make sure, these two enabled: true values are populated in the configuration.yaml file of the host agent:
...
Let's move to the Instana UI and make sure, the agent is reporting content to the Instana backend. Open the Instana UI in the browser and navigate to Settings -> Agents as shown on the below screen:
