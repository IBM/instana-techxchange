...
From the Student host system open the Firefox browser and select the **Instana**
bookmark.
...
dropdown and select **Generic Webhook**
...
After selecting  **Generic Webhook**  the screen will change to show the fields required to
configure the **Generic Webhook** Alert Channel. An Alert Channel can be setup to
notify specific users or to notify a large groups of users. For example, you
might want all database related Events to be sent to a _database team webhook
alias_ for the DBAs. Or, you might want to notify a specific line of business or
the IT Operations team.

Here are the steps for setting up the **Generic Webhook** channel.
...
- Enter a name for the alert. The name can be anything, but should be
  ...
  ![](images/newAlertChannel.png)

- We will use the https://webhook.site link to create a generic webhook. Open 
  a new tab on the browser and enter the url https://webhook.site. Copy the Webhook 
  URL in the **Your Unique URL** field

  ![](images/webhooksite.png)

- Enter the URL in the Webhook URL field in the Instana Alert Channel configuration page
  and select **Test Channel** 

  ![](images/webhookInstanaTest.png)

  :::tip

  If you see a connectivity error, retry the test to make sure you see the **Test Successful** message

  :::


- Verify a test Webhook message is sent on the webhook.site page

  ![](images/webhooksite_test.png)
...
to the **Alert Channel**. In this case, a Webhook will be sent.
