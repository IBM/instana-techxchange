...
### 4.3 Creating a scripted REST API test
...
For the test we need the Instana token that we created on previous step.  If you didn't save the API Token, you can use the following steps to retrieve the token.
   ...
   - Paste Token as value for TOKEN variable (line 8 on screenshot above)

After you had input the correct API Token, then select the PoP that you had just installed in previous lab exercise.
...
Give the test a meaningful name and description:
...
It may take up to a few minutes for Instana to perform the first test and show the line with results. You can change the refresh time duration for the dashboard on top right of the window (e.g. click on LIVE) to show the test name or the results of the test as soon as they are are processed.

By now you should see a list of tests:
![](images/ListOfTests-new.png)

Click on the test name **Instana REST API check** to access the dashboard of the synthetic test:

![](images/SyntheticTestDashboard-new.png)

At the moment we have 3 or 4 agents reporting to the backend.  In the javascript, the assert parameter is checking whether there are more than 4 Hosts connected, so the check has passed. 
...
:::info
In the event that the javascript made multiple REST calls, the synthetic results page will also show each of the REST calls and their corresponding timings as well.
In the following screenshot example from some other demo system (not part of this lab exercise), the Results page shows response timings for each of the REST calls:

![](images/MultipleRESTcall.png)

as well as the breakdown of the response timings.

![](images/RESTtimingBreakdown.png)

:::

By now, you had learned how to create both a Simple REST API test and a Scripted REST API test in Instana's Synthetic Monitoring. 
Instana can also support more advanced API script, which you can refer to this URL for further information.
