...
1.2 Add the following 5 feature flags in spec -> featureFlags section to enable Synthetics features:
```yaml
spec:
  featureFlags:
  - enabled: true
...
1.4 If Helm wasn't installed on previous labs, install it on bastion node now using:
```bash
...
2.2.6 Instana will run the script from previously installed Synthetic Point of Presence, they can be placed at multiple places across available infrastructure. The one we will use is deployed on the same cluster with Instana, it may have different name. Select it for our test.
...
2.2.11 Repeat the creation step 2.2 for additional "Failing test" with the script adjusted as `assert.ok(bodyObj.hostCount < 1,` at the last request instead of `< 2` as below:
