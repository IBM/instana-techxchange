...
Do NOT copy and paste the helm chart from the lab guide.  You MUST get the helm command from the Instana server
because it has information such as the instanaKey that ties it to your Instana server.
...
These parameters help identify the Point of Presence when there are multiples deployed.  NOTE:   The controller location MUST have exactly 7 parameters that are separated by ";".
...
The helm chart should now look similar to the following:
