...
Instana include a capability called the **Action Catalog** which allows you to execute corrective actions in order to solve problems in an automated way.  
...
Next, create on **Create** button to create your new action.
...
The configuration.yaml file of the agent running on your **student** node had already enabled for Actions. If time permits, you can view the configuration.yaml file from this directory:
```bash
...
Go back to the Instana user interface.  Within the Action Catalog, you will see your Action.  Notice that you can manually execute the Action by selecting the 3dot icon, followed by **Test**
...
```yml
com.instana.plugin.action.nsible:
  enabled: true  # by default is false
...
When this Automation Action Ansible sensor is enabled, it will automatically create a docker container which will host the automation connector to communicate with the Ansible controller. It will then sync the playbooks from Ansible into Instana's Action Catalog.
...
Now, let's verify the RobotShop catalog is indeed stop by refreshing the homepage of **Stan's Robot Shop**. You will notice that the all the Categories listing is missing, as the catalog's docker container is down.

![](images/RobotShop_withoutCatalog.png)

Next, let's start back again the RobotShop catalog container again. From the **Action Catalog**, click on the 3dots icon at the far right of **Start RobotShop Catalog**, and select **Test**
